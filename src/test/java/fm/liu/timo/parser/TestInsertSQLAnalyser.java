...
public class TestInsertSQLAnalyser {
	...
	public void testInsertSQL() throws SQLSyntaxErrorException {
		...
		Assert.assertEquals("table1".toUpperCase(), parsInf.getTable()
				.getIdTextUpUnescape());
	 ...
	 * @param ddl
	 */
	private void enrichAutoIncrementColumn(
			Identifier columnAutoIncrementColumn, DMLInsertStatement ddl) {
		boolean isEnrichAutoIncrementColumn = true;
		for (Identifier column : ddl.getColumnNameList()) {
			if (column.getIdTextUpUnescape().equals(
					columnAutoIncrementColumn.getIdTextUpUnescape())) {
