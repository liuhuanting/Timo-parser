...
public class MySQLDMLInsertParserTest extends AbstractSyntaxTest {
	...
	public void testInsert() throws SQLSyntaxErrorException {
		...
		MySQLDMLInsertParser parser = new MySQLDMLInsertParser(lexer,
		...
		Assert.assertEquals(
				"INSERT HIGH_PRIORITY INTO test.t1 (t1.id1, id2) VALUES (?, '123')",
				output);
		...
		output = output2MySQL(insert, sql);
		Assert.assertEquals(
				"INSERT INTO t1 VALUES (123, ?) ON DUPLICATE KEY UPDATE ex.col1 = ?",
				output);
		...
		output = output2MySQL(insert, sql);
		Assert.assertEquals(
				"INSERT LOW_PRIORITY INTO t1 VALUES (0.12, 1, 2), (?), (DEFAULT)",
				output);
		...
		parser.match(MySQLToken.EOF);
		output = output2MySQL(insert, sql);
		Assert.assertEquals("INSERT LOW_PRIORITY INTO t1 SELECT id FROM t1",
				output);
		...
		output = output2MySQL(insert, sql);
		Assert.assertEquals(
				"INSERT LOW_PRIORITY INTO t1 (t1.col1) VALUES (123), ('12\\'34')",
				output);
		...
		output = output2MySQL(insert, sql);
		Assert.assertEquals(
				"INSERT LOW_PRIORITY IGNORE INTO t1 (col1) SELECT id FROM t3",
				output);
