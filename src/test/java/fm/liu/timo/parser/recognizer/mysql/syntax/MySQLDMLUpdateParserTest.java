...
public class MySQLDMLUpdateParserTest extends AbstractSyntaxTest {
	...
	public void testUpdate() throws SQLSyntaxErrorException {
		...
		MySQLDMLUpdateParser parser = new MySQLDMLUpdateParser(lexer,
				new MySQLExprParser(lexer));
		DMLUpdateStatement update = parser.update();
		String output = output2MySQL(update, sql);
		Assert.assertNotNull(update);
		Assert.assertEquals(
				"UPDATE LOW_PRIORITY IGNORE test.t1 SET t1.col1 = ?, col2 = DEFAULT",
				output);
		...
		Assert.assertEquals(
				"UPDATE IGNORE t1 SET col2 = DEFAULT ORDER BY t1.col2", output);
		...
		output = output2MySQL(update, sql);
		Assert.assertEquals(
				"UPDATE test.t1 SET col2 = DEFAULT ORDER BY t1.col2 LIMIT 1 , 2",
				output);
		...
		output = output2MySQL(update, sql);
		Assert.assertEquals(
				"UPDATE LOW_PRIORITY t1, test.t2 SET col2 = DEFAULT, col2 = '123\\'4'",
				output);
