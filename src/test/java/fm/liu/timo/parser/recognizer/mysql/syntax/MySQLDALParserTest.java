...
public class MySQLDALParserTest extends AbstractSyntaxTest {
	...
	public void testdesc() throws Exception {
		...
		MySQLDALParser parser = new MySQLDALParser(lexer, new MySQLExprParser(
				lexer));
	...
	public void testSet() throws Exception {
		...
		parser.match(MySQLToken.EOF);
		output = output2MySQL(set, sql);
		Assert.assertEquals("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED",
				output);
		...
		output = output2MySQL(set, sql);
		Assert.assertEquals(
				"SET GLOBAL TRANSACTION ISOLATION LEVEL READ COMMITTED", output);
		...
		parser.match(MySQLToken.EOF);
		output = output2MySQL(set, sql);
		Assert.assertEquals("SET TRANSACTION ISOLATION LEVEL REPEATABLE READ",
				output);
		...
		output = output2MySQL(set, sql);
		Assert.assertEquals(
				"SET SESSION TRANSACTION ISOLATION LEVEL SERIALIZABLE", output);
	...
	public void testShow() throws Exception {
		...
		parser.match(MySQLToken.EOF);
		output = output2MySQL(show, sql);
		Assert.assertEquals("SHOW BINLOG EVENTS IN 'a' FROM 1 LIMIT 1 , 2",
				output);
		...
		output = output2MySQL(show, sql);
		Assert.assertEquals(
				"SHOW CHARACTER SET WHERE (SELECT a FROM tb) IN (a)", output);
		...
		output = output2MySQL(show, sql);
		Assert.assertEquals(
				"SHOW COLLATION WHERE AVG(SELECT * FROM tb1, tb2) != 100",
				output);
		...
		output = output2MySQL(show, sql);
		Assert.assertEquals(
				"SHOW FULL COLUMNS FROM db1.tb1 WHERE COUNT(col) > 10", output);
		...
		output = output2MySQL(show, sql);
		Assert.assertEquals(
				"SHOW DATABASES WHERE (SELECT * FROM `select`) IS NOT NULL",
				output);
		...
		parser.match(MySQLToken.EOF);
		output = output2MySQL(show, sql);
		Assert.assertEquals("SHOW OPEN TABLES FROM db WHERE tb IS NOT NULL",
				output);
		...
		parser.match(MySQLToken.EOF);
		output = output2MySQL(show, sql);
		Assert.assertEquals("SHOW PROCEDURE STATUS WHERE (a OR b) * (a AND b)",
				output);
		...
		Assert.assertEquals(
				"SHOW PROFILE ALL, BLOCK IO, CONTEXT SWITCHES, CPU, IPC, MEMORY, "
						+ "PAGE FAULTS, SOURCE, SWAPS FOR QUERY 2 LIMIT 2 , 1",
				output);
		...
		output = output2MySQL(show, sql);
		Assert.assertEquals(
				"SHOW TABLE STATUS FROM db WHERE (SELECT a) > (SELECT b)",
				output);
		...
		parser.match(MySQLToken.EOF);
		output = output2MySQL(show, sql);
		Assert.assertEquals("SHOW TABLE STATUS FROM db WHERE id1 = a OR b",
				output);
		...
		output = output2MySQL(show, sql);
		Assert.assertEquals(
				"SHOW FULL TABLES FROM db WHERE id1 = ABS(SELECT a)", output);
		...
		output = output2MySQL(show, sql);
		Assert.assertEquals(
				"SHOW TRIGGERS FROM db WHERE STRCMP('test1', 'test2')", output);
		...
		parser.match(MySQLToken.EOF);
		output = output2MySQL(show, sql);
		Assert.assertEquals("SHOW SESSION VARIABLES WHERE a * b + 1 = c",
				output);
