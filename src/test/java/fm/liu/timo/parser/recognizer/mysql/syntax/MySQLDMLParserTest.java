...
public class MySQLDMLParserTest extends AbstractSyntaxTest {
	...
	public void testOrderBy() throws SQLSyntaxErrorException {
		...
		ListUtil.isEquals(ListUtil.createList(new Pair<Expression, SortOrder>(
				new Identifier(null, "c1"), SortOrder.ASC),
				new Pair<Expression, SortOrder>(new Identifier(null, "c2"),
						SortOrder.DESC), new Pair<Expression, SortOrder>(
						new Identifier(null, "c3"), SortOrder.ASC)), orderBy
				.getOrderByList());
		...
		ListUtil.isEquals(ListUtil.createList(new Pair<Expression, SortOrder>(
				new Identifier(null, "c1"), SortOrder.ASC)), orderBy
				.getOrderByList());
		...
		output = output2MySQL(orderBy, sql);
		ListUtil.isEquals(ListUtil.createList(new Pair<Expression, SortOrder>(
				new Identifier(null, "c1"), SortOrder.DESC)), orderBy
				.getOrderByList());
	...
	public void testLimit() throws SQLSyntaxErrorException {
		...
		Assert.assertEquals(1, limit.getOffset());
		Assert.assertEquals(6, limit.getSize());
		Assert.assertEquals("LIMIT 1 , 6", output);
		...
		limit = parser.limit();
		output = output2MySQL(limit, sql);
		Assert.assertEquals(3, limit.getOffset());
		Assert.assertEquals(9, limit.getSize());
		Assert.assertEquals("LIMIT 3 , 9", output);
	...
	public void testGroupBy() throws SQLSyntaxErrorException {
		...
		ListUtil.isEquals(ListUtil.createList(new Pair<Expression, SortOrder>(
				...
				new Pair<Expression, SortOrder>(new Identifier(null, "c2"),
						SortOrder.DESC), new Pair<Expression, SortOrder>(
						new Identifier(null, "c3"), SortOrder.ASC)), groupBy
				.getOrderByList());
		...
		output = output2MySQL(groupBy, sql);
		ListUtil.isEquals(ListUtil.createList(new Pair<Expression, SortOrder>(
				new Identifier(null, "c1"), SortOrder.ASC)), groupBy
				.getOrderByList());
		...
		output = output2MySQL(groupBy, sql);
		ListUtil.isEquals(ListUtil.createList(new Pair<Expression, SortOrder>(
				new Identifier(null, "c1"), SortOrder.DESC)), groupBy
				.getOrderByList());
	...
	public void testTR1() throws SQLSyntaxErrorException {
		...
		Assert.assertEquals(1, list.size());
		Assert.assertEquals(TableRefFactor.class, list.get(0).getClass());
		Expression ex = ((InnerJoin) (trs.getTableReferenceList()).get(0))
				.getOnCond();
		Assert.assertEquals(ex.getClass(), ComparisionEqualsExpression.class);
		Assert.assertEquals(
				"(tb1 AS t1) INNER JOIN (tb2 AS t2) ON t1.name = t2.name",
				output);
		...
		trs = parser.tableRefs();
		output = output2MySQL(trs, sql);
		List<String> using_list = ((InnerJoin) (trs.getTableReferenceList())
				.get(0)).getUsing();
		Assert.assertEquals(1, using_list.size());
		Assert.assertEquals("(tb1 AS t1) INNER JOIN tb2 AS t2 USING (c1)",
				output);
		...
		trs = parser.tableRefs();
		output = output2MySQL(trs, sql);
		using_list = ((InnerJoin) (trs.getTableReferenceList()).get(0))
				.getUsing();
		Assert.assertEquals(2, using_list.size());
		Assert.assertEquals("(tb1 AS t1) INNER JOIN tb2 AS t2 USING (c1, c2)",
				output);
		...
		Assert.assertEquals(1, list.size());
		Assert.assertEquals(TableRefFactor.class, list.get(0).getClass());
		List<IndexHint> hintlist = ((TableRefFactor) (trs
				.getTableReferenceList()).get(0)).getHintList();
		...
		Assert.assertEquals(2, list.size());
		Assert.assertEquals(TableRefFactor.class, list.get(0).getClass());
		hintlist = ((TableRefFactor) (trs.getTableReferenceList()).get(0))
				.getHintList();
		Assert.assertEquals(1, hintlist.size());
		...
		Assert.assertEquals("USE", indexhint.getAction().name());
		Assert.assertEquals("INDEX", indexhint.getType().name());
		hintlist = ((TableRefFactor) (trs.getTableReferenceList()).get(1))
				.getHintList();
		Assert.assertEquals(1, hintlist.size());
		indexhint = hintlist.get(0);
		Assert.assertEquals(3, indexhint.getIndexList().size());
		...
		Assert.assertEquals(1, list.size());
		Assert.assertEquals(TableRefFactor.class, list.get(0).getClass());
		Assert.assertEquals("tb1", ((TableRefFactor) (trs
				.getTableReferenceList()).get(0)).getTable().getIdText());
		Assert.assertEquals("t1", ((TableRefFactor) (trs
				.getTableReferenceList()).get(0)).getAlias());
		...
		Assert.assertEquals("tb1", ((TableRefFactor) (trs
				.getTableReferenceList()).get(0)).getTable().getIdText());
		Assert.assertEquals(null, ((TableRefFactor) (trs
				.getTableReferenceList()).get(0)).getAlias());
		Assert.assertEquals("tb2", ((TableRefFactor) (trs
				.getTableReferenceList()).get(1)).getTable().getIdText());
		Assert.assertEquals("tb3", ((TableRefFactor) (trs
				.getTableReferenceList()).get(2)).getTable().getIdText());
		...
		Assert.assertEquals("tb1 USE KEY FOR ORDER BY (i1, i2) "
				+ "USE KEY FOR GROUP BY () IGNORE INDEX FOR GROUP BY (i1, i2)",
				output);
		...
		Assert.assertEquals("GROUP_BY", indexhint.getScope().name());
		Assert.assertEquals(
				"tb1 USE INDEX FOR ORDER BY (i1, i2) FORCE INDEX FOR GROUP BY (i1)",
				output);
		...
		TableReferences ltr = (TableReferences) ((OuterJoin) list.get(0))
				.getLeftTableRef();
		Assert.assertEquals(1, ltr.getTableReferenceList().size());
		Assert.assertEquals(TableRefFactor.class, ltr.getTableReferenceList()
				.get(0).getClass());
		Assert.assertEquals(null, ((TableRefFactor) (ltr
				.getTableReferenceList().get(0))).getAlias());
		Assert.assertEquals("tb1", ((TableRefFactor) (ltr
				.getTableReferenceList().get(0))).getTable().getIdText());
		hintlist = ((TableRefFactor) (ltr.getTableReferenceList()).get(0))
				.getHintList();
		Assert.assertEquals(1, hintlist.size());
		indexhint = hintlist.get(0);
		Assert.assertEquals(2, indexhint.getIndexList().size());
		...
		Assert.assertEquals("INDEX", indexhint.getType().name());
		Assert.assertEquals("JOIN", indexhint.getScope().name());
		TableRefFactor rtf = (TableRefFactor) ((OuterJoin) list.get(0))
				.getRightTableRef();
		...
		Assert.assertEquals(
				"(tb1 FORCE INDEX FOR JOIN (i1, i2)) "
		...
		Assert.assertEquals(TableReferences.class, ltr.getTableReferenceList()
				.get(0).getClass());
		TableReferences ltr1 = (TableReferences) (ltr.getTableReferenceList())
				.get(0);
		Assert.assertEquals(2, ltr1.getTableReferenceList().size());
		Assert.assertEquals(TableReferences.class, ltr1.getTableReferenceList()
				.get(0).getClass());
		TableReferences ltr2 = (TableReferences) (ltr1.getTableReferenceList())
				.get(0);
		Assert.assertEquals(2, ltr2.getTableReferenceList().size());
		Assert.assertEquals(TableRefFactor.class, ltr2.getTableReferenceList()
				.get(0).getClass());
		Assert.assertEquals(null, ((TableRefFactor) (ltr2
				.getTableReferenceList().get(0))).getAlias());
		Assert.assertEquals("tb1", ((TableRefFactor) (ltr2
				.getTableReferenceList().get(0))).getTable().getIdText());
		hintlist = ((TableRefFactor) (ltr2.getTableReferenceList()).get(0))
				.getHintList();
		Assert.assertEquals(1, hintlist.size());
		indexhint = hintlist.get(0);
		Assert.assertEquals(2, indexhint.getIndexList().size());
		...
		Assert.assertEquals(TableRefFactor.class, ltr2.getTableReferenceList()
				.get(1).getClass());
		Assert.assertEquals("tb3", ((TableRefFactor) (ltr2
				.getTableReferenceList().get(1))).getTable().getIdText());
		Assert.assertEquals(TableRefFactor.class, ltr1.getTableReferenceList()
				.get(1).getClass());
		Assert.assertEquals("tb4", ((TableRefFactor) (ltr1
				.getTableReferenceList().get(1))).getTable().getIdText());
		Assert.assertEquals(TableRefFactor.class, ltr.getTableReferenceList()
				.get(1).getClass());
		Assert.assertEquals("tb5", ((TableRefFactor) (ltr
				.getTableReferenceList().get(1))).getTable().getIdText());
		TableReferences rtr = (TableReferences) ((OuterJoin) list.get(0))
				.getRightTableRef();
		Assert.assertEquals("t2", ((TableRefFactor) rtr.getTableReferenceList()
				.get(0)).getAlias());
		Assert.assertEquals("tb2", ((TableRefFactor) rtr
				.getTableReferenceList().get(0)).getTable().getIdText());
		hintlist = ((TableRefFactor) rtr.getTableReferenceList().get(0))
				.getHintList();
		Assert.assertEquals(1, hintlist.size());
		indexhint = hintlist.get(0);
		Assert.assertEquals(3, indexhint.getIndexList().size());
		...
		Assert.assertEquals("INDEX", indexhint.getType().name());
		Assert.assertEquals("ALL", indexhint.getScope().name());
		using_list = ((OuterJoin) (trs.getTableReferenceList()).get(0))
				.getUsing();
		Assert.assertEquals(1, using_list.size());
		Assert.assertEquals(
		...
		output = output2MySQL(trs, sql);
		Assert.assertEquals(
				"tb1 AS t1 INNER JOIN tb2 AS t2 USE INDEX (i1) USING (id1)",
				output);
		...
		output = output2MySQL(trs, sql);
		Assert.assertEquals(
				"tb1 AS _latin't1' INNER JOIN tb2 AS t2 USE INDEX (i1)", output);
		...
		Assert.assertEquals(1, list.size());
		Assert.assertEquals(InnerJoin.class, list.get(0).getClass());
		SubqueryFactor lsf = (SubqueryFactor) ((InnerJoin) list.get(0))
				.getLeftTableRef();
		Assert.assertEquals("t1", lsf.getAlias());
		Assert.assertEquals(DMLSelectStatement.class, lsf.getSubquery()
				.getClass());
		...
		trs = parser.tableRefs();
		output = output2MySQL(trs, sql);
		Assert.assertEquals("tb1 STRAIGHT_JOIN tb2 AS t2 ON tb1.id = tb2.id",
				output);
		...
		Assert.assertEquals(1, ltr.getTableReferenceList().size());
		TableRefFactor lltrf = (TableRefFactor) ((OuterJoin) ltr
				.getTableReferenceList().get(0)).getLeftTableRef();
		Assert.assertEquals(null, lltrf.getAlias());
		Assert.assertEquals("tb1", lltrf.getTable().getIdText());
		using_list = ((OuterJoin) ltr.getTableReferenceList().get(0))
				.getUsing();
		...
		Assert.assertEquals("tb3", ((TableRefFactor) (rtr
				.getTableReferenceList().get(0))).getTable().getIdText());
		Assert.assertEquals("tb4", ((TableRefFactor) (rtr
				.getTableReferenceList().get(1))).getTable().getIdText());
		Assert.assertEquals(LogicalAndExpression.class,
				((InnerJoin) list.get(0)).getOnCond().getClass());
		Assert.assertEquals(
				"tb1 RIGHT JOIN tb2 USING (id1, id2) "
		...
		trs = parser.tableRefs();
		output = output2MySQL(trs, sql);
		Assert.assertEquals("tb1 LEFT JOIN (tb2 INNER JOIN tb3) USING (id)",
				output);
		...
		NaturalJoin lnj = (NaturalJoin) ((NaturalJoin) list.get(0))
				.getLeftTableRef();
		...
		Assert.assertEquals(null, rtf.getAlias());
		Assert.assertEquals("tb3", rtf.getTable().getIdText());
		Assert.assertEquals("tb1 NATURAL RIGHT JOIN tb2 NATURAL LEFT JOIN tb3",
				output);
		...
		Assert.assertEquals(1, list.size());
		Assert.assertEquals(OuterJoin.class, list.get(0).getClass());
		Assert.assertEquals(TableReferences.class, ((OuterJoin) list.get(0))
				.getLeftTableRef().getClass());
		Assert.assertEquals(InnerJoin.class, ((OuterJoin) list.get(0))
				.getRightTableRef().getClass());
		...
		Assert.assertEquals(TableRefFactor.class, sj.getRightTableRef()
				.getClass());
		...
		Assert.assertEquals(InnerJoin.class, ((OuterJoin) (list.get(0)))
				.getLeftTableRef().getClass());
		Assert.assertEquals(StraightJoin.class, ((OuterJoin) (list.get(0)))
				.getRightTableRef().getClass());
		StraightJoin rsj = (StraightJoin) ((OuterJoin) (list.get(0)))
				.getRightTableRef();
		Assert.assertEquals(TableRefFactor.class, rsj.getLeftTableRef()
				.getClass());
		Assert.assertEquals(TableReferences.class, rsj.getRightTableRef()
				.getClass());
		list = ((TableReferences) rsj.getRightTableRef())
				.getTableReferenceList();
		Assert.assertEquals(OuterJoin.class, list.get(0).getClass());
		Assert.assertEquals(
