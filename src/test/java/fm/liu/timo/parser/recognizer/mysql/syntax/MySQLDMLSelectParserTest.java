...
public class MySQLDMLSelectParserTest extends AbstractSyntaxTest {
	...
	public static void main(String[] ars) throws Exception {
		...
		for (int i = 0; i < 3; ++i) {
			...
			MySQLDMLSelectParser parser = new MySQLDMLSelectParser(lexer,
					exprParser);
	...
	public void testSelectUnion() throws SQLSyntaxErrorException {
		String sql = "(select id from t1) union all (select id from t2) union all (select id from t3) ordeR By d desC limit 1 offset 5";
		MySQLLexer lexer = new MySQLLexer(sql);
		MySQLDMLSelectParser parser = new MySQLDMLSelectParser(lexer,
				new MySQLExprParser(lexer));
		DMLSelectUnionStatement select = (DMLSelectUnionStatement) parser
				.selectUnion();
		...
		Assert.assertEquals(
				"(SELECT id FROM t1) UNION ALL (SELECT id FROM t2) UNION ALL (SELECT id FROM t3) ORDER BY d DESC LIMIT 5 , 1",
				output);
	...
	public void testSelect() throws SQLSyntaxErrorException {
		...
		output = output2MySQL(select, sql);
		Assert.assertEquals(
				"SELECT tb1.id, tb2.id FROM tb1, tb2 WHERE tb1.id2 = tb2.id2",
				output);
		...
		output = output2MySQL(select, sql);
		Assert.assertEquals(
				"SELECT SQL_CACHE id1, id2 FROM tb1, tb2 WHERE tb1.id1 = tb2.id1",
				output);
		...
		Assert.assertNotNull(select);
		output = output2MySQL(select, sql);
		Assert.assertEquals("SELECT SQL_BUFFER_RESULT tb1.id1, id2 FROM tb1",
				output);
		...
		Assert.assertNotNull(select);
		output = output2MySQL(select, sql);
		Assert.assertEquals("SELECT SQL_CALC_FOUND_ROWS tb1.id1, id2 FROM tb1",
				output);
