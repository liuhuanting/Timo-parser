...
public class MySQLDDLParserTest extends AbstractSyntaxTest {
	...
	public void testTruncate() throws Exception {
		...
		MySQLDDLParser parser = new MySQLDDLParser(lexer, new MySQLExprParser(
				lexer));
	...
	public void testDDLStmt() throws Exception {
		...
		Assert.assertEquals(
				"DROP TEMPORARY TABLE IF EXISTS tb1, tb2, tb3 RESTRICT", output);
		...
		output = output2MySQL(dst, sql);
		Assert.assertEquals(
				"DROP TEMPORARY TABLE IF EXISTS tb1, tb2, tb3 CASCADE", output);
		...
		dst = parser.ddlStmt();
		output = output2MySQL(dst, sql);
		Assert.assertEquals("DROP TEMPORARY TABLE IF EXISTS tb1 CASCADE",
				output);
