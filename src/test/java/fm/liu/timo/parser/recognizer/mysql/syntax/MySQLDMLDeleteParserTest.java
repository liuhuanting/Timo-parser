...
public class MySQLDMLDeleteParserTest extends AbstractSyntaxTest {
	public void testDelete1() throws SQLSyntaxErrorException {
		...
		MySQLDMLDeleteParser parser = new MySQLDMLDeleteParser(lexer,
				new MySQLExprParser(lexer));
		DMLDeleteStatement delete = parser.delete();
		parser.match(MySQLToken.EOF);
		String output = output2MySQL(delete, sql);
		Assert.assertEquals(
				"DELETE LOW_PRIORITY id1.id, id FROM t1 AS a WHERE col1 = ?",
				output);
		...
		output = output2MySQL(delete, sql);
		Assert.assertEquals(
				"DELETE offer.*, wp_image.* FROM offer AS a, wp_image AS b WHERE "
						+ "a.member_id = b.member_id AND a.member_id = 'abc'",
				output);
		...
		output = output2MySQL(delete, sql);
		Assert.assertEquals(
				"DELETE FROM id1.id WHERE col1 = 'adf' LIMIT 1 , 1", output);
		...
		output = output2MySQL(delete, sql);
		Assert.assertEquals(
				"DELETE FROM id WHERE col1 = 'adf' ORDER BY d LIMIT 2 , 2",
				output);
		...
		output = output2MySQL(delete, sql);
		Assert.assertEquals(
				"DELETE id.* FROM t1, t2 WHERE col1 = 'adf' AND col2 = 1",
				output);
		...
		output = output2MySQL(delete, sql);
		Assert.assertEquals(
				"DELETE FROM t1 WHERE t1.id1 = 'abc' ORDER BY a LIMIT 0 , 5",
				output);
