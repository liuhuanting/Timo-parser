...
public class OutputVisitor extends Visitor {
	...
	/**
	 * @param args
	 *            parameters for {@link java.sql.PreparedStatement preparedStmt}
	 */
	...
	public void visit(UnaryOperatorExpression node) {
		...
		boolean paren = node.getOperand().getPrecedence() < node
				.getPrecedence();
		if (paren)
			appendable.append('(');
	...
	public void visit(BinaryOperatorExpression node) {
		...
		Expression left = node.getLeftOprand();
		boolean paren = node.isLeftCombine() ? left.getPrecedence() < node
				.getPrecedence() : left.getPrecedence() <= node.getPrecedence();
		if (paren)
			appendable.append('(');
		left.accept(this);
		if (paren)
		...
		Expression right = node.getRightOprand();
		paren = node.isLeftCombine() ? right.getPrecedence() <= node
				.getPrecedence() : right.getPrecedence() < node.getPrecedence();
		if (paren)
			appendable.append('(');
		right.accept(this);
		if (paren)
	...
	public void visit(Trim node) {
		...
		case TRAILING:
			...
			break;
		default:
			throw new IllegalArgumentException("unknown trim direction: "
					+ node.getDirection());
		}
	...
	public void visit(Extract node) {
		appendable.append("EXTRACT(").append(node.getUnit().name())
				.append(" FROM ");
		printList(node.getArguments());
		appendable.append(')');
	}
	...
	public void visit(Timestampdiff node) {
		appendable.append("TIMESTAMPDIFF(").append(node.getUnit().name())
				.append(", ");
		printList(node.getArguments());
		appendable.append(')');
	}
	...
	public void visit(Timestampadd node) {
		appendable.append("TIMESTAMPADD(").append(node.getUnit().name())
				.append(", ");
		printList(node.getArguments());
		appendable.append(')');
	}
	...
	private static boolean containsCompIn(Expression pat) {
		...
		if (pat instanceof BinaryOperatorExpression) {
			...
			return containsCompIn(tp.getFirst())
					|| containsCompIn(tp.getSecond())
	...
	public void visit(MatchExpression node) {
		...
		default:
			throw new IllegalArgumentException(
					"unkown modifier for match expression: "
							+ node.getModifier());
		}
		appendable.append(')');
	}
	...
	public void visit(SysVarPrimary node) {
		...
		default:
			throw new IllegalArgumentException(
					"unkown scope for sysVar primary: " + scope);
		}
	...
	public void visit(IndexHint node) {
		...
		default:
			throw new IllegalArgumentException(
					"unkown index action for index hint: " + action);
		}
		...
		default:
			throw new IllegalArgumentException(
					"unkown index type for index hint: " + type);
		}
		...
		default:
			throw new IllegalArgumentException(
					"unkown index scope for index hint: " + scope);
		}
		appendable.append('(');
	...
	public void visit(ColumnDefinition node) {
		throw new UnsupportedOperationException(
				"col_def in CREATE TABLE is partially parsed");
	}
	...
	public void visit(AlterSpecification node) {
		throw new UnsupportedOperationException(
				"subclass have not implement visit");
	}
	...
	public void visit(ShowCreate node) {
		appendable.append("SHOW CREATE ").append(node.getType().name())
				.append(' ');
	...
	public void visit(ShowStatus node) {
		appendable.append("SHOW ")
				.append(node.getScope().name().replace('_', ' '))
				.append(" STATUS");
		printLikeOrWhere(node.getPattern(), node.getWhere());
	}
	...
	public void visit(MTSSetTransactionStatement node) {
		...
		if (scope != null) {
			...
			default:
				throw new IllegalArgumentException(
						"unknown scope for SET TRANSACTION ISOLATION LEVEL: "
								+ scope);
			}
		}
		...
		default:
			throw new IllegalArgumentException(
					"unknown level for SET TRANSACTION ISOLATION LEVEL: "
							+ node.getLevel());
		}
	}
	...
	public void visit(MTSRollbackStatement node) {
		...
		if (savepoint == null) {
			...
			default:
				throw new IllegalArgumentException(
						"unrecgnized complete type: " + type);
			}
		} else {
	...
	public void visit(DMLInsertStatement node) {
		...
		case UNDEF:
			break;
		default:
			throw new IllegalArgumentException("unknown mode for INSERT: "
					+ node.getMode());
		}
		if (node.isIgnore())
			appendable.append("IGNORE ");
	...
	public void visit(DMLSelectStatement node) {
		...
		case DISTINCTROW:
			...
			break;
		default:
			throw new IllegalArgumentException("unknown option for SELECT: "
					+ option);
		}
	...
	public void visit(DMLSelectUnionStatement node) {
		...
		if (list == null || list.isEmpty()) {
			throw new IllegalArgumentException(
					"SELECT UNION must have at least one SELECT");
		}
	...
	public void visit(DDLAlterTableStatement node) {
		throw new UnsupportedOperationException(
				"ALTER TABLE is partially parsed");
	}

	@Override
	public void visit(DDLCreateIndexStatement node) {
		throw new UnsupportedOperationException(
				"CREATE INDEX is partially parsed");
	}
