...
public class MySQLLexer {
	 ...
	 */
	protected void updateStringValue(final char[] src, final int srcOffset,
			final int len) {
		...
		for (; srcIndex < end; ++srcIndex) {
			...
			if (c >= 'a' && c <= 'z') {
				...
				if (srcIndex > srcOffset) {
					System.arraycopy(src, srcOffset, sbuf, 0, srcIndex
							- srcOffset);
				}
				break;
			}
	...
	/**
	 * @param skip
	 *            if 1, then equals to {@link #scanChar()}
	 */
	...
	/**
	 * @param quoteMode
	 *            if false: first <code>0x</code> has been skipped; if true:
	 *            first <code>x'</code> has been skipped
	 */
	protected void scanHexaDecimal(boolean quoteMode)
	...
	/**
	 * @param quoteMode
	 *            if false: first <code>0b</code> has been skipped; if true:
	 *            first <code>b'</code> has been skipped
	 */
	protected void scanBitField(boolean quoteMode)
			throws SQLSyntaxErrorException {
		offsetCache = curIndex;
			...
			case 3:
				if (CharTypes.isDigit(ch)) {
					...
					if (CharTypes.isIdentifierChar(ch)) {
						...
						updateStringValue(sql, offsetCache, sizeCache);
						MySQLToken tok = keywods
								.getKeyword(stringValueUppercase);
						token = tok == null ? MySQLToken.IDENTIFIER : tok;
					}
					return;
				} else {
					throw err("invalid char after '.' and 'e' for as part of number: "
							+ ch);
				}
				break;
			case 4:
	 ...
	 * 
	 * @param initSize
	 *            how many char has already been consumed
	 */
	 ...
	 */
	protected SQLSyntaxErrorException err(String msg)
			throws SQLSyntaxErrorException {
	...
	public String toString() {
		...
		sb.append("curIndex=").append(curIndex).append(", ch=").append(ch)
				.append(", token=").append(token).append(", sqlLeft=")
				.append(sqlLeft).append(", sql=").append(sql);
		sb.append('}');
		return sb.toString();
	}
