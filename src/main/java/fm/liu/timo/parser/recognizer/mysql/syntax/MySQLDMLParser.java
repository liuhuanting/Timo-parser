...
public abstract class MySQLDMLParser extends MySQLParser {
	 ...
	 *            never null
	 */
	protected List<Identifier> buildIdList(Identifier id)
			...
			case KW_LEFT:
				...
				default:
					...
					if (condition instanceof Expression) {
						ref = new OuterJoin(isLeft, ref, temp,
	...
	private IndexHint hint() throws SQLSyntaxErrorException {
		...
		case KW_KEY:
			...
			break;
		default:
			throw err("must be INDEX or KEY for hint type, not "
					+ lexer.token());
		}
		...
		if (lexer.nextToken() == KW_FOR) {
			...
			case KW_GROUP:
				...
				break;
			default:
				throw err("must be JOIN or ORDER or GROUP for hint scope, not "
						+ lexer.token());
			}
		}
