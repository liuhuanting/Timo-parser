...
public class MySQLDMLSelectParser extends MySQLDMLParser {
	...
	static {
		specialIdentifiers.put("SQL_BUFFER_RESULT",
				SpecialIdentifier.SQL_BUFFER_RESULT);
	...
	private DMLSelectStatement.SelectOption selectOption()
		...
		for (DMLSelectStatement.SelectOption option = new DMLSelectStatement.SelectOption();; lexer
			...
			case IDENTIFIER:
				...
				SpecialIdentifier specialId = specialIdentifiers
						.get(optionStringUp);
				if (specialId != null) {
					switch (specialId) {
	...
	private List<Pair<Expression, String>> selectExprList()
		...
		for (; lexer.token() == PUNC_COMMA; list
				.add(new Pair<Expression, String>(expr, alias))) {
			lexer.nextToken();
	...
	public DMLSelectStatement select() throws SQLSyntaxErrorException {
		...
		return new DMLSelectStatement(option, exprList, tables, where, group,
				having, order, limit);
	}
