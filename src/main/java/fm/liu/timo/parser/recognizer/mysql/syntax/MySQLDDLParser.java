...
public class MySQLDDLParser extends MySQLParser {
	...
	static {
		...
		specialIdentifiers.put("KEY_BLOCK_SIZE",
				SpecialIdentifier.KEY_BLOCK_SIZE);
		...
		specialIdentifiers.put("AUTO_INCREMENT",
				SpecialIdentifier.AUTO_INCREMENT);
		specialIdentifiers.put("AVG_ROW_LENGTH",
				SpecialIdentifier.AVG_ROW_LENGTH);
		...
		specialIdentifiers.put("DELAY_KEY_WRITE",
				SpecialIdentifier.DELAY_KEY_WRITE);
		specialIdentifiers
				.put("INSERT_METHOD", SpecialIdentifier.INSERT_METHOD);
	...
	public DDLStatement ddlStmt() throws SQLSyntaxErrorException {
		...
		case KW_ALTER:
			...
			case KW_TABLE:
				lexer.nextToken();
				idTemp1 = identifier();
				DDLAlterTableStatement alterTableStatement = new DDLAlterTableStatement(
						ignore, idTemp1);
			...
			case IDENTIFIER:
				...
				if (siTemp != null) {
					...
					case POLICY:
						...
						ExtDDLCreatePolicy policy = new ExtDDLCreatePolicy(
								policyName);
		...
		case KW_RENAME:
			...
			return new DDLRenameTableStatement(list);
		case IDENTIFIER:
			SpecialIdentifier si = specialIdentifiers.get(lexer
	 ...
	 */
	private DDLDropTableStatement dropTable(boolean temp)
		...
		for (int i = 0; lexer.token() != EOF; ++i) {
			...
			case KW_DROP:
				...
				case KW_KEY:
					...
					lexer.nextToken();
					id = identifier();
					stmt.addAlterSpecification(new DDLAlterTableStatement.DropIndex(
							id));
					break drop_switch;
				case KW_PRIMARY:
					// | DROP PRIMARY KEY
				...
				case IDENTIFIER:
					...
					id = identifier();
					stmt.addAlterSpecification(new DDLAlterTableStatement.DropColumn(
							id));
					break drop_switch;
				case KW_COLUMN:
					// | DROP [COLUMN] col_name
			...
			case KW_CHANGE:
				...
				if (lexer.token() == IDENTIFIER) {
					if ("FIRST".equals(lexer.stringValueUppercase())) {
						lexer.nextToken();
						stmt.addAlterSpecification(new DDLAlterTableStatement.ChangeColumn(
								id, id2, colDef, null));
			...
			case KW_ADD:
				...
				case KW_KEY:
					...
					stmt.addAlterSpecification(new DDLAlterTableStatement.AddIndex(
							id, indexDef));
					break add_switch;
				case KW_PRIMARY:
				...
				case KW_UNIQUE:
					...
					indexDef = indexDefinition();
					stmt.addAlterSpecification(new DDLAlterTableStatement.AddUniqueKey(
							id, indexDef));
					break add_switch;
				case KW_FULLTEXT:
	 ...
	 * <code>TABLE</code> has been consumed
	 */
	private DDLCreateTableStatement createTable(boolean temp)
			throws SQLSyntaxErrorException {
		...
		DDLCreateTableStatement stmt = new DDLCreateTableStatement(temp,
				ifNotExists, table);
		...
		DMLSelectStatement select = new MySQLDMLSelectParser(lexer, exprParser)
				.select();
	...
	private void createTableDefs(DDLCreateTableStatement stmt)
		...
		for (int i = 0; lexer.token() != PUNC_RIGHT_PAREN; ++i) {
			...
			default:
				throw new SQLSyntaxErrorException(
						"unsupportted column definition");
			}
		}
		match(PUNC_RIGHT_PAREN);
	}
	...
	private IndexDefinition indexDefinition() throws SQLSyntaxErrorException {
		...
		if (lexer.token() == KW_USING) {
			...
			indexType = tp == 0 ? IndexDefinition.IndexType.BTREE
					: IndexDefinition.IndexType.HASH;
		}
		if (lexer.token() == IDENTIFIER) {
			lexer.nextToken();
		}
	...
	private List<IndexOption> indexOptions() throws SQLSyntaxErrorException {
		...
		for (;;) {
			...
			case KW_USING:
				lexer.nextToken();
				IndexOption.IndexType indexType = matchIdentifier("BTREE",
						"HASH") == 0 ? IndexOption.IndexType.BTREE
			...
			case IDENTIFIER:
				...
				if (si != null) {
					...
					case COMMENT:
						lexer.nextToken();
						LiteralString string = (LiteralString) exprParser
								.expression();
						if (list == null) {
							list = new ArrayList<IndexOption>(1);
						}
	...
	private DataType dataType() throws SQLSyntaxErrorException {
		...
		return new DataType(typeName, unsigned, zerofill, binary, length,
				decimals, charSet, collation, collectionVals);
	}
	...
	private ColumnDefinition columnDefinition() throws SQLSyntaxErrorException {
		...
		if (lexer.token() == IDENTIFIER
				&& "AUTO_INCREMENT".equals(lexer.stringValueUppercase())) {
			lexer.nextToken();
			autoIncrement = true;
		}
		switch (lexer.token()) {
		...
		if (lexer.token() == IDENTIFIER
				&& "COMMENT".equals(lexer.stringValueUppercase())) {
			lexer.nextToken();
			comment = (LiteralString) exprParser.expression();
		}
		if (lexer.token() == IDENTIFIER
				&& "COLUMN_FORMAT".equals(lexer.stringValueUppercase())) {
			switch (lexer.nextToken()) {
			case KW_DEFAULT:
				lexer.nextToken();
		...
		return new ColumnDefinition(dataType, notNull, defaultVal,
				autoIncrement, sindex, comment, format);
	}

	private boolean tableOptions(TableOptions options)
			throws SQLSyntaxErrorException {
	...
	private boolean tableOption(TableOptions options)
			throws SQLSyntaxErrorException {
		Identifier id = null;
		Expression expr = null;
		...
		case IDENTIFIER:
			...
			if (si != null) {
				...
				case CONNECTION:
					...
					options.setConnection((LiteralString) exprParser
							.expression());
					break os;
				case DATA:
				...
				case ROW_FORMAT:
					...
					case KW_DEFAULT:
						...
						break os;
					case IDENTIFIER:
						SpecialIdentifier sid = specialIdentifiers.get(lexer
								.stringValueUppercase());
						if (sid != null) {
							switch (sid) {
							case DYNAMIC:
