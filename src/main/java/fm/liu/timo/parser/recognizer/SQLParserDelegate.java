...
public final class SQLParserDelegate {
	...
	private static String buildErrorMsg(Exception e, MySQLLexer lexer,
			String sql) {
		...
		sb.append('{').append(fragment).append('}')
				.append(". Error cause: " + e.getMessage());
		return sb.toString();
	}

	public static SQLStatement parse(String sql, MySQLLexer lexer,
			String charset) throws SQLSyntaxErrorException {
		try {
			...
			case PUNC_LEFT_PAREN:
				stmt = new MySQLDMLSelectParser(lexer, exprParser)
						.selectUnion();
				break stmtSwitch;
			case KW_DELETE:
			...
			default:
				throw new SQLSyntaxErrorException(
						"sql is not a supported statement");
			}
			if (isEOF) {
