...
public class MySQLMTSParser extends MySQLParser {
	...
	public MTSRollbackStatement rollback() throws SQLSyntaxErrorException {
		...
		// follow:
		lexer.nextToken();
		SpecialIdentifier siTemp = specialIdentifiers.get(lexer
		...
		case EOF:
			return new MTSRollbackStatement(
					MTSRollbackStatement.CompleteType.UN_DEF);
		case KW_TO:
			lexer.nextToken();
		...
		case KW_AND:
			...
			if (siTemp == SpecialIdentifier.NO) {
				...
				match(EOF);
				return new MTSRollbackStatement(
						MTSRollbackStatement.CompleteType.NO_CHAIN);
			}
			matchIdentifier("CHAIN");
			match(EOF);
			return new MTSRollbackStatement(
					MTSRollbackStatement.CompleteType.CHAIN);
		case KW_RELEASE:
			lexer.nextToken();
			match(EOF);
			return new MTSRollbackStatement(
		...
		case IDENTIFIER:
			...
			if (siTemp == SpecialIdentifier.NO) {
				...
				match(EOF);
				return new MTSRollbackStatement(
						MTSRollbackStatement.CompleteType.NO_RELEASE);
			}
		default:
