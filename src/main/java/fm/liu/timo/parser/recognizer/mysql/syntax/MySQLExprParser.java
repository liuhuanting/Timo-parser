...
public class MySQLExprParser extends MySQLParser {
	public MySQLExprParser(MySQLLexer lexer) {
		this(lexer, MySQLFunctionManager.INSTANCE_MYSQL_DEFAULT, true,
				DEFAULT_CHARSET);
	}
	...
	public MySQLExprParser(MySQLLexer lexer,
			MySQLFunctionManager functionManager, boolean cacheEvalRst,
			String charset) {
		...
		for (Expression fst = bitOrExpression(null, null);;) {
			...
			case KW_NOT:
				...
				case KW_IN:
					...
					fst = new InExpression(true, fst, in)
							.setCacheEvalRst(cacheEvalRst);
					continue;
				default:
			...
			case KW_REGEXP:
				lexer.nextToken();
				temp = bitOrExpression(null, null);
				fst = new RegexpExpression(false, fst, temp)
						.setCacheEvalRst(cacheEvalRst);
				continue;
			case KW_LIKE:
				lexer.nextToken();
			...
			case KW_IN:
				...
				fst = new InExpression(false, fst, temp)
						.setCacheEvalRst(cacheEvalRst);
				continue;
			case KW_IS:
			...
			case OP_EQUALS:
				...
				fst = new ComparisionEqualsExpression(fst, temp)
						.setCacheEvalRst(cacheEvalRst);
				continue;
			...
			default:
				if (equalsIdentifier("SOUNDS") >= 0) {
					...
					match(KW_LIKE);
					temp = bitOrExpression(null, null);
					fst = new SoundsLikeExpression(fst, temp)
							.setCacheEvalRst(cacheEvalRst);
					continue;
				}
				return fst;
	...
	private Expression anyAllExpression() throws SQLSyntaxErrorException {
		...
		case KW_ALL:
			...
			subquery = subQuery();
			match(PUNC_RIGHT_PAREN);
			return new SubqueryAllExpression(subquery)
					.setCacheEvalRst(cacheEvalRst);
		default:
			...
			if (lexer.nextToken() == PUNC_LEFT_PAREN) {
				...
				subquery = subQuery();
				match(PUNC_RIGHT_PAREN);
				return new SubqueryAnyExpression(subquery)
						.setCacheEvalRst(cacheEvalRst);
			}
	...
	/**
	 * @param consumed
	 *            not null means that a token that has been pre-consumed stands
	 *            for next token
	 */
	...
	private Expression bitShiftExpression(String consumed, String consumedUp)
		...
		for (Expression expr = arithmeticTermOperatorExpression(consumed,
			...
			case OP_LEFT_SHIFT:
				...
				expr = new BitShiftExpression(false, expr, temp)
						.setCacheEvalRst(cacheEvalRst);
				break;
			case OP_RIGHT_SHIFT:
				lexer.nextToken();
				temp = arithmeticTermOperatorExpression(null, null);
				expr = new BitShiftExpression(true, expr, temp)
						.setCacheEvalRst(cacheEvalRst);
				break;
			default:
				return expr;
	 ...
	 * <code>higherExpr ( ('+'|'-') higherExpr)+</code>
	 */
	private Expression arithmeticTermOperatorExpression(String consumed,
			String consumedUp) throws SQLSyntaxErrorException {
		Expression temp;
		for (Expression expr = arithmeticFactorOperatorExpression(consumed,
			...
			case OP_PLUS:
				...
				expr = new ArithmeticAddExpression(expr, temp)
						.setCacheEvalRst(cacheEvalRst);
				break;
			case OP_MINUS:
				lexer.nextToken();
	 ...
	 */
	private Expression arithmeticFactorOperatorExpression(String consumed,
			String consumedUp) throws SQLSyntaxErrorException {
		Expression temp;
		for (Expression expr = bitXORExpression(consumed, consumedUp);;) {
			...
			case OP_SLASH:
				lexer.nextToken();
				temp = bitXORExpression(null, null);
				expr = new ArithmeticDivideExpression(expr, temp)
						.setCacheEvalRst(cacheEvalRst);
				break;
			case KW_DIV:
				lexer.nextToken();
			...
			case KW_MOD:
				lexer.nextToken();
				temp = bitXORExpression(null, null);
				expr = new ArithmeticModExpression(expr, temp)
						.setCacheEvalRst(cacheEvalRst);
				break;
			default:
				return expr;
	...
	private Expression bitXORExpression(String consumed, String consumedUp)
		...
		for (Expression expr = unaryOpExpression(consumed, consumedUp);;) {
			...
			case OP_CARET:
				...
				expr = new BitXORExpression(expr, temp)
						.setCacheEvalRst(cacheEvalRst);
				break;
			default:
				return expr;
	...
	private Expression collateExpression(String consumed, String consumedUp)
		...
		for (Expression expr = userExpression(consumed, consumedUp);;) {
			if (lexer.token() == KW_COLLATE) {
				...
				match(IDENTIFIER);
				expr = new CollateExpression(expr, collateName)
						.setCacheEvalRst(cacheEvalRst);
				continue;
			}
			return expr;
	...
	private Expression userExpression(String consumed, String consumedUp)
		...
		if (lexer.token() == USR_VAR) {
			if (first instanceof LiteralString) {
				...
				lexer.nextToken();
				return new UserExpression(str.toString())
						.setCacheEvalRst(cacheEvalRst);
			} else if (first instanceof Identifier) {
				StringBuilder str = new StringBuilder().append(
						((Identifier) first).getIdText()).append(
						lexer.stringValue());
				lexer.nextToken();
				return new UserExpression(str.toString())
						.setCacheEvalRst(cacheEvalRst);
	...
	private Expression primaryExpression(final String consumed,
			String consumedUp) throws SQLSyntaxErrorException {
		if (consumed != null) {
		...
		case LITERAL_BIT:
			tempStr = lexer.stringValue();
			lexer.nextToken();
			return new LiteralBitField(null, tempStr)
					.setCacheEvalRst(cacheEvalRst);
		case LITERAL_HEX:
			LiteralHexadecimal hex = new LiteralHexadecimal(null,
					lexer.getSQL(), lexer.getOffsetCache(),
		...
		case USR_VAR:
			tempStr = lexer.stringValue();
			tempExpr = new UsrDefVarPrimary(tempStr)
					.setCacheEvalRst(cacheEvalRst);
		...
		case KW_DEFAULT:
			if (lexer.nextToken() == PUNC_LEFT_PAREN) {
				return ordinaryFunction(lexer.stringValue(),
		...
		case KW_VALUES:
			...
			throw err("keyword not followed by '(' is not expression: "
					+ tempStr);
		case KW_MOD:
			lexer.nextToken();
			match(PUNC_LEFT_PAREN);
	...
	private Char functionChar() throws SQLSyntaxErrorException {
		...
		for (List<Expression> tempExprList = new LinkedList<Expression>();;) {
			...
			case KW_USING:
				...
				return chr;
			default:
				throw err("expect ',' or 'USING' or ')' but is "
						+ lexer.token());
			}
		}
	}
	 ...
	 * <code>null</code>
	 */
	private Expression startedFromIdentifier(final String consumed,
			String consumedUp) throws SQLSyntaxErrorException {
		Expression tempExpr;
		Expression tempExpr2;
		List<Expression> tempExprList;
			...
			for (tempExpr = new Identifier(null, consumed, consumedUp)
				...
				case IDENTIFIER:
					tempExpr = new Identifier((Identifier) tempExpr,
							lexer.stringValue(), lexer.stringValueUppercase())
							.setCacheEvalRst(cacheEvalRst);
					lexer.nextToken();
					break;
				case OP_ASTERISK:
					...
					return new Wildcard((Identifier) tempExpr)
							.setCacheEvalRst(cacheEvalRst);
				default:
					throw err("expect IDENTIFIER or '*' after '.', but is "
							+ lexer.token());
				}
			}
			return tempExpr;
		case LITERAL_BIT:
			...
			tempStr = lexer.stringValue();
			lexer.nextToken();
			return new LiteralBitField(consumed, tempStr)
					.setCacheEvalRst(cacheEvalRst);
		case LITERAL_HEX:
			if (consumed.charAt(0) != '_') {
				return new Identifier(null, consumed, consumedUp)
		...
		case PUNC_LEFT_PAREN:
			...
			case GEOMETRYFROMTEXT:
				tempExpr = expression();
				return new GeometryFromText(tempExpr)
						.setCacheEvalRst(cacheEvalRst);
			case ISCLOSED:
				tempExpr = expression();
			...
			case MBRCONTAINS:
				...
				tempExprList.add(expression());
				match(PUNC_RIGHT_PAREN);
				return new MBRContains(tempExprList)
						.setCacheEvalRst(cacheEvalRst);
			case MBREQUAL:
				lexer.nextToken();
				tempExprList = new ArrayList<Expression>(2);
			...
			case MBRWITHIN:
				...
				tempExprList.add(expression());
				match(PUNC_RIGHT_PAREN);
				return new MBRWithin(tempExprList)
						.setCacheEvalRst(cacheEvalRst);
			case SRID:
				tempExpr = expression();
			...
			case ST_CONTAINS:
				...
				tempExprList.add(expression());
				match(PUNC_RIGHT_PAREN);
				return new ST_Contains(tempExprList)
						.setCacheEvalRst(cacheEvalRst);
			case ST_DISTANCE:
				lexer.nextToken();
				tempExprList = new ArrayList<Expression>(2);
				...
				tempExprList.add(expression());
				match(PUNC_RIGHT_PAREN);
				return new ST_Distance(tempExprList)
						.setCacheEvalRst(cacheEvalRst);
			case ST_EQUALS:
				lexer.nextToken();
				tempExprList = new ArrayList<Expression>(2);
				...
				tempExprList.add(expression());
				match(PUNC_RIGHT_PAREN);
				return new ST_Equals(tempExprList)
						.setCacheEvalRst(cacheEvalRst);
			case ST_WITHIN:
				lexer.nextToken();
				tempExprList = new ArrayList<Expression>(2);
				...
				tempExprList.add(expression());
				match(PUNC_RIGHT_PAREN);
				return new ST_Within(tempExprList)
						.setCacheEvalRst(cacheEvalRst);
			case WITHIN:
				lexer.nextToken();
				tempExprList = new ArrayList<Expression>(2);
			...
			case GET_FORMAT:
				...
				lexer.nextToken();
				int gfi = matchIdentifier("DATE", "TIME", "DATETIME",
						"TIMESTAMP");
				match(PUNC_COMMA);
				...
				switch (gfi) {
				case 0:
					return new GetFormat(GetFormat.FormatType.DATE,
							getFormatArg);
				case 1:
					return new GetFormat(GetFormat.FormatType.TIME,
							getFormatArg);
				case 2:
				case 3:
					return new GetFormat(GetFormat.FormatType.DATETIME,
							getFormatArg);
				}
			...
			case CAST:
				...
				if (info != null) {
					return new Cast(tempExpr, type.getKey(), info.getKey(),
							info.getValue()).setCacheEvalRst(cacheEvalRst);
				} else {
			...
			case GROUP_CONCAT:
				...
				case KW_SEPARATOR:
					...
					tempSb = new StringBuilder();
					lexer.appendStringContent(tempSb);
					tempStr = LiteralString.getUnescapedString(tempSb
							.toString());
					match(LITERAL_CHARS);
					break;
				default:
				...
				match(PUNC_RIGHT_PAREN);
				return new GroupConcat(tempGroupDistinct, tempExprList,
						tempExpr, isDesc, appendedColumnNames, tempStr)
						.setCacheEvalRst(cacheEvalRst);
			case CHAR:
				lexer.nextToken();
				return functionChar();
	...
	private static Pair<String, Pair<Expression, Expression>> constructTypePair(
			String typeName, Expression exp1, Expression exp2) {
	...
	private IntervalPrimary.Unit intervalPrimaryUnit()
		...
		case IDENTIFIER:
			...
			IntervalPrimary.Unit unit = IntervalPrimary
					.getIntervalUnit(unitText);
			if (unit != null) {
				lexer.nextToken();
				return unit;
			}
