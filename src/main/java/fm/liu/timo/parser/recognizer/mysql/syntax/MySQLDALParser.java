...
public class MySQLDALParser extends MySQLParser {
	...
	static {
		...
		specialIdentifiers.put("PERFORMANCE_SCHEMA",
				SpecialIdentifier.PERFORMANCE_SCHEMA);
	...
	private ShowIndex showIndex(ShowIndex.Type type)
	 ...
	 * </pre>
	 */
	private ShowColumns showColumns(boolean full)
			throws SQLSyntaxErrorException {
		lexer.nextToken();
		match(KW_FROM, KW_IN);
		Identifier table = identifier();
	...
	private String getStringValue() throws SQLSyntaxErrorException {
		...
		case LITERAL_CHARS:
			...
			name = LiteralString.getUnescapedString(name.substring(1,
					name.length() - 1));
			lexer.nextToken();
			return name;
		default:
	...
	public SQLStatement set() throws SQLSyntaxErrorException {
		...
		if (lexer.token() == IDENTIFIER
				&& SpecialIdentifier.NAMES == specialIdentifiers.get(lexer
		...
		if (lexer.token() != PUNC_COMMA) {
			assignmentList = new ArrayList<Pair<VariableExpression, Expression>>(
					1);
	 ...
	 */
	private MTSSetTransactionStatement setMTSSetTransactionStatement(
			VariableScope scope) throws SQLSyntaxErrorException {
		lexer.nextToken();
	...
	private Object varAssign() throws SQLSyntaxErrorException {
		...
		case IDENTIFIER:
			...
			var = new SysVarPrimary(scope, lexer.stringValue(),
