...
public abstract class MySQLParser {
	...
	public Identifier identifier() throws SQLSyntaxErrorException {
		...
		case OP_ASTERISK:
			...
			return wc;
		case IDENTIFIER:
			id = new Identifier(null, lexer.stringValue(),
		...
		for (; lexer.token() == PUNC_DOT;) {
			...
			case OP_ASTERISK:
				...
				return wc;
			case IDENTIFIER:
				id = new Identifier(id, lexer.stringValue(),
						lexer.stringValueUppercase());
				id.setCacheEvalRst(cacheEvalRst);
				lexer.nextToken();
				break;
	...
	/**
	 * @param expectTextUppercase
	 *            must be upper-case
	 * @return index (start from 0) of expected text which is first matched. -1
	 *         if none is matched.
	 */
	protected int equalsIdentifier(String... expectTextUppercases)
			throws SQLSyntaxErrorException {
	 ...
	 */
	protected int matchIdentifier(String... expectTextUppercase)
			throws SQLSyntaxErrorException {
		...
		for (int i = 0; i < expectTextUppercase.length; ++i) {
			if (id == null ? expectTextUppercase[i] == null : id
					.equals(expectTextUppercase[i])) {
				lexer.nextToken();
				return i;
			}
		}
	 ...
	 *             if no token is matched
	 */
	protected int match(MySQLToken... expectToken)
			throws SQLSyntaxErrorException {
	...
	protected SQLSyntaxErrorException err(String msg)
		...
		errmsg.append(msg).append(". lexer state: ")
				.append(String.valueOf(lexer));
